...
parser.add_argument('--save-dir', default='/checkpoints/local-error/', type=str,
...
class LocalLossInvertedBlockConv(nn.Module):
    '''
    ...
    '''
    def __init__(self, ch_in, ch_out, kernel_size, stride, padding, num_classes, first_layer=False, dropout=None, bias=None, pre_act=False, post_act=True):
        super(LocalLossInvertedBlockConv, self).__init__()
        ...
        if args.loss_unsup == 'sim' or args.loss_sup == 'sim' or:
    ...
    def forward(self, x, y, y_onehot, x_shortcut=None):
        ...
        # Calculate local loss and update weights
        if not args.no_print_stats or self.training:
            ...
            if args.loss_unsup == 'sim' or args.loss_sup == 'sim':
...
cfg_invertedNN = {
    'inn6':[
        64, "M", 128, "M", 256, "M", 256],
}

class InvertedNN(nn.Module):
    '''
    Inverted Neural Network makes its prediction starting from a small downsampled image and gradually accumulates recognized patterns 
    by making more detailed feature maps in every heirarch with larger
    inputs. This is similar to SinGAN architecture. From the hiearchy of 
    features perspective starting from coarse representations and build
    upon them seems more intuitive for now.
    ...
    '''
    def __init__(self, config, input_dim, input_ch, num_classes, feat_mult=1):
        super(VGGn, self).__init__()
        self.cfg = config
        self.input_dim = input_dim
        self.input_ch = input_ch
        self.num_classes = num_classes
        self.features, output_dim = self._make_layers(self.config, input_ch, input_dim, feat_mult)
        ...
        if args.num_layers > 0: 
            ...
            self.classifier = nn.Linear(input_dim*input_dim*int(output_ch * feat_mult), num_classes)
    ...
    def _make_layers(self, config, input_ch, input_dim, feat_mult):
        layers = []
        first_layer = True
        ...
        for c in config:
            if c == "U":
                if input_dim/scale_cum <= 4:
                    break
                layers += [nn.Upsample2d(scale_factor=2, mode='nearest')]
                scale_cum *=2

            x = int(c * feat_mult)
            elif first_layer and input_dim > 64:
                scale_cum = 2
                layers += [ Conv2d(image_channels, x, kernel_size=7, stride=2, padding=3),
                            LocalLossInvertedBlockConv(input_ch, x, kernel_size=7, stride=2, padding=3, 
                                         num_classes=num_classes, 
                                         first_layer=first_layer)]
            else:
                layers += [ Conv2d(image_channels, x, kernel_size=3, stride=1, padding=1),
                            LocalLossInvertedBlockConv(input_ch, x, kernel_size=3, stride=1, padding=1, 
                                         num_classes=num_classes, 
                                         first_layer=first_layer)]
